              ...
              <ElCol :span="12" class="wrap">
                  <el-card class="el-card__main">
                      ...
                      <draggable v-model="tasks" @change="dragAndDrop">
                          <taskItem v-for="task in filterTasks" :id="task.id" :name="task.name" :description="task.description" :status="task.status" :key="task.id" @deleteTask="deleteTask" @editTask="editTask" @changeStatus="changeTaskStatus" class="task-item" />
                      </draggable>
